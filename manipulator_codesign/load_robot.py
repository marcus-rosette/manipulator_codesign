...
class LoadRobot:
    ...
    #     joint_positions = list(joint_positions)
    #     w0 = self.calculate_manipulability(joint_positions)
    #     grad = np.zeros_like(joint_positions)
    #     for i in range(len(joint_positions)):
    #         q_delta = np.array(joint_positions, dtype=float)
    #         q_delta[i] += delta
    #         w1 = self.calculate_manipulability(q_delta)
    #         grad[i] = (w1 - w0) / delta
    #     return grad
    ...
    # def resolved_rate_control(self, target_pose, alpha=0.1, max_steps=100, tol=1e-3, manipulability_gain=0.01):
    #     """
    #     Resolved-rate motion control with manipulability maximization.
        
    #     Args:
    #         robot (LoadRobot): Instance of the robot interface
    #         target_pose (tuple): (target_position, target_orientation)
    #         alpha (float): Step size for joint updates
    #         max_steps (int): Max iteration steps
    #         tol (float): Position/orientation tolerance
    #         manipulability_gain (float): Scaling factor for manipulability gradient term
    #     """
    #     for step in range(max_steps):
    #         q = np.array(self.get_joint_positions())
    ...
    #         ori_axis, ori_angle = self.con.getAxisAngleFromQuaternion(ori_err)

    #         vel_ee = np.hstack((pos_err, ori_axis * ori_angle))

    #         # Manipulability bias term (nullspace projection)
    ...
    #         q_new = q + alpha * dq
    #         self.set_joint_configuration(q_new.tolist())

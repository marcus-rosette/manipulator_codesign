...
class PlanarPruner:
    ...
    def check_pose_within_tolerance(self, final_position, final_orientation, target_position, target_orientation, pos_tolerance, ori_tolerance):
        ...
        return (pos_diff <= pos_tolerance and ori_diff <= ori_tolerance, pos_diff, ori_diff)
...
def main():
    ...
    goal_orientation = p.getQuaternionFromEuler([-1.17, 0, 0])
    pos_tolerance = 0.5
    ori_tolerance = 1.5
    ...
    for i in range(max_iterations):
        ...
        if path is not None:
            ...
            if within_tol:
                ...
                print(f'Manipulability at final position: {manipulability}')
                # print("")
                # break
            else:
                path = None
